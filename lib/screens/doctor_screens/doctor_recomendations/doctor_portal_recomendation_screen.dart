...
import 'package:ifeelin_color/controllers/doctor_controllers/recomendation_controllers/doctor_portal_recomendation_controller.dart';
...
class DoctorPortalRecomendationScreen extends StatefulWidget {
  final String? patientId;
  const DoctorPortalRecomendationScreen({super.key, required this.patientId});

  @override
  State<DoctorPortalRecomendationScreen> createState() =>
      _DoctorPortalRecomendationScreenState();
}

class _DoctorPortalRecomendationScreenState
    extends State<DoctorPortalRecomendationScreen> {
  final DoctorPortalRecomendationController controller =
      Get.put(DoctorPortalRecomendationController());
  @override
  void initState() {
    super.initState();
    controller.fetchRecommendations(widget.patientId);
  }
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      return controller.isLoading.value
          ...
          : controller.totalportalRecomendations.value == 0
              ? const Center(child: Text('No Portal Recomendations'))
              : ListView.builder(
                  shrinkWrap: true,
                  itemCount: controller.totalportalRecomendations.value,
                  padding:
                      const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
                  itemBuilder: (context, index) {
                    var allRecomendations = controller
                        .doctorPortalRecomendationsModel?.body?[index];
                                            ...
                                            child: Padding(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  Text(
                                                    'category- ${allRecomendations?.type}',
                                                    style: Theme.of(context)
                                                        .textTheme
                                                        .bodySmall,
                                                  ),
