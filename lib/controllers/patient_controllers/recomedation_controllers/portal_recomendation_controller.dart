...
import 'package:ifeelin_color/models/patient_models/recomendation_models/portal_recomendation_model.dart';
...
class PortalRecomendationController extends GetxController {
  ...
  PortalRecomendationModel? portalRecomendationModel;

  var totalportalRecomendations = Rxn<int>().obs;
  ...
  Future<void> fetchRecommendations() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${Constants.baseUrl}/${Constants.portalRecommendations}'),
        headers: {
      ...
      if (response.statusCode == 200) {
        portalRecomendationModel =
            portalRecomendationModelFromJson(response.body);
        totalportalRecomendations.value(portalRecomendationModel?.body?.length);
        update();
  ...
  void updateItems([int? index, String? category]) {
    if (index != null && category != null) {
      switch (category) {
        case 'images':
          items[index] = portalRecomendationModel
